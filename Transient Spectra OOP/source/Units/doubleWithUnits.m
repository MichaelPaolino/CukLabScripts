classdef doubleWithUnits < double
    ...
    methods
        ...
        % Constructs a DOUBLEWITHUNITS object. This object extends the double class
        % by keeping track of double array units and includes user-defined
        % conversion rules. 
        %
        ...
        %
        % obj = DOUBLEWITHUNITS()
        %   Default constructor with an empty array with no user defined units
        %
        % obj = DOUBLEWITHUNITS(data)
        %   Constructor with a double data array but no user defined units
        %
        % obj = DOUBLEWITHUNITS(data,obj)
        %   Constructor with a double data array and user defined units/conversion
        %   copied from obj, which is an existing DOUBLEWITHUNITS.
        %
        % obj = DOUBLEWITHUNITS(data,newUnit,newUnitDisplayName)
        %   Constructor with a double data array with user assigned base unit. The
        %   base unit must have a short name (called in conversion) and a display
        %   name. The recommended format for display name is unit type followed by
        %   matlab LaTeX in parentheses, e.g. for energy: Wavenumber (cm^{-1}) 

            %handles adding unitless double array data by caling double superclass constructor
