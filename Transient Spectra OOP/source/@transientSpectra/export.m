...
% in the igor project. 
%
% The base name for each matrix or wave is determined by obj.shortName
...
%   each matrix and wave that is saved
    ...
    for objInd = 1:objNumel
        ...
        saveFlag = struct('delays',   obj(objInd).sizes.nDelays > 1,...  %true if multiple delays
        ...
        [wlSubVal, wlSubInds] = nearestVal(obj(objInd).wavelengths,wlSub,'trim',false,'threshold',0.01*mean(obj(objInd).wavelengths,'all'));
        [delaySubVal, delaySubInds] = nearestVal(obj(objInd).delays,delaySub,'trim',false);    
        ...
        dataName = nameRule({'shortName',  {obj(objInd).shortName},                                                          0, nameFlag.shortName && ~isempty(obj(objInd).shortName);...
                             'label',      {''},                                                                     0, true;...
                             'dataUnit',   {['in ' unitStr.spectra]},                                                0, nameFlag.spectraUnit;...
                             'scheme',     obj(objInd).schemes,                                                              1, (obj(objInd).sizes.nSchemes>1) || nameFlag.scheme;...
                             'gPos',       strcat({'gPos: '},strcat(strtrim(cellstr(num2str(obj(objInd).gPos(:))))), ' nm'), 2, (obj(objInd).sizes.nGPos>1) || nameFlag.gPos;...
                             'rpts',       strcat({'rpt '}, strtrim(cellstr(num2str((1:obj(objInd).sizes.nRpts)')))),        3, (obj(objInd).sizes.nRpts>1) || nameFlag.rpts;...
        ...
        if saveFlag.contours
            ...
            nContours = obj(objInd).sizes.nRpts*obj(objInd).sizes.nGPos*obj(objInd).sizes.nSchemes;
            contours = repmat(struct('name','','data',[]),nContours,1);

            %Populate countours structure with matrix and matrix name
            cc = 1;    
            for ii = 1:obj(objInd).sizes.nSchemes
                for jj = 1:obj(objInd).sizes.nGPos
                    for kk = 1:obj(objInd).sizes.nRpts                   
                        ...
                        contours(cc).data = obj(objInd).spectra.data(:,:,kk,jj,ii);
        ...
        if saveFlag.spectra
            ...
            %Loop over data groups to do name conversion
            nWaves = obj(objInd).sizes.nRpts*obj(objInd).sizes.nGPos*obj(objInd).sizes.nSchemes*nSubDelays;
            waves = repmat(struct('name','','data',[]),nWaves,1);

            %Populate countours structure with matrix and matrix name
            cc = 1;    
            for schemeInd = 1:obj(objInd).sizes.nSchemes
                    ...
                    for rptInd = 1:obj(objInd).sizes.nRpts
                        ...
                        for delayInd = delaySubInds(:,rptInd,gPosInd)'
                            if ~isnan(delayInd)
                                ...
                                waves(cc).data = obj(objInd).spectra.data(:,delayInd,rptInd,gPosInd,schemeInd);
        ...
        if saveFlag.traces
            ...
            %Loop over data groups to do name conversion
            nWaves = obj(objInd).sizes.nRpts*obj(objInd).sizes.nGPos*obj(objInd).sizes.nSchemes*nSubWls;
            waves = repmat(struct('name','','data',[]),nWaves,1);
                    ...
                    for rptInd = 1:obj(objInd).sizes.nRpts
                        for wlInd = wlSubInds(:,gPosInd)'
                            if ~isnan(wlInd)    %If the wavelength is outside the grating position it is flagged with NaN
                                ...
                                waves(cc).data = obj(objInd).spectra.data(wlInd,:,rptInd,gPosInd,schemeInd).';
        ...
        if saveFlag.delays
            ...
            %Loop over data groups to do name conversion
            nWaves = obj(objInd).sizes.nRpts*obj(objInd).sizes.nGPos;
            waves = repmat(struct('name','','data',[]),nWaves,1);
            ...
            for gPosInd = 1:obj(objInd).sizes.nGPos
                for rptInd = 1:obj(objInd).sizes.nRpts
                    ...
                    waves(cc).data = obj(objInd).delays.data(:,rptInd,gPosInd);
        ...
        if saveFlag.wavelengths
            ...
            for gPosInd = 1:obj(objInd).sizes.nGPos
                    ...
                    waves(cc).data = obj(objInd).wavelengths.data(:,gPosInd);
