...
% DBSTAGE generates tables that will be commited to the database based on 
% user input and the object array contents. This method does not actually 
% save data or commit data. Use obj.dbCommit(__) to save and commit data.
%
% [stageTable] = obj.dbStage(connObj, dbTableName)
...
%   the target database table. procdTable contains columns, 'FileName',
%   'FilePath','ShortName','FileTypeID', and 'TimeStamp'. The Filename is 
%   autogenerated from the time stamp (now) and obj.shortName. The default 
%   file path is the current working directory. File type is always set to
%   'tsObj'.
%
%   Note: this method requires that dbTableName contains the same column 
%   names as returned by procdTable.
%
% [procdTable] = obj.dbStage(__, 'name1','value1',...)
%   Same call as above but with optional name-value pairs:
%   'path': char array specifying the data save path. Default is the
%       current working directory.
%   'prefix': char array specifying a prefix to add to shortName before
%       generating file paths and the ShortName column. Default is no
%       prefix.
%   'colStruct': a structure that sets values for columns in dbTableName
%       for all object elements. The structure field name is the column 
%       name and field value is the column value. 
%       Example: if you want to set a column named isDataGood to true for 
%       all rows, you would pass struct('isDataGood', true) as the value 
%       argument.
%   'fkStruct': a structure array that allows the user to set foreign key
%       columns by values in the primary key table. This adds the foreign 
%       key column to procdTable. Use genFKStruct to generate the structure
...
% See Also: GENFKSTRUCT, DATABASE
            ...
            tNow.Format = 'yyyy-MM-dd HH:mm:ss'; % for database
        ...
        function commitTable = dbCommitSave(obj, conn, stageTable) 
% DBCOMMITSAVE commits object data to file and the database based on the
% entries given by the output of obj.dbStageSave(...).
            ...
            conn.sqlwrite(stageTable.Properties.Description,stageTable);
            
            % Query the database for the commited table for return argument
            commitTable = conn.fetch(['SELECT TOP ', num2str(size(stageTable,1)) ' * FROM ',  stageTable.Properties.Description, ' ORDER BY ID DESC;']);
            commitTable = sortrows(commitTable,'ID');
