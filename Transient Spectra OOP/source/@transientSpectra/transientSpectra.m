...
% See Also: SPLITSCHEMES
            ...
            for objInd = 1:extraDims
                ...
                % Update name (filepath) to contain a cell array of names
                mergedObj(objInd).name = {obj(:,objInd).name};
        ...
        %       'shift': data from each grating position is shifted either up or down 
        %       based on whether it lies above or below the midpoint between the data 
        %       in the overlap region. The amount of shift is based on the average of 
        %       all data across the overlap wavelength region for each grating postion.
        %
        % See Also: AVERAGE, INTERP, MERGE
            
            %optional inputs: strategy for stitching. Default is linear
            p = inputParser();
            p.addOptional('strategy','linear',@(s) ischar(s) && any(strcmp(s,{'average','lower','upper','half','linear','shift'})));
            p.parse(varargin{:});
                ...
                if size(wl,2) > 1
                    for ii = 2:size(wl,2) %loop over grating positions starting from the 2nd one
                        ...
                        switch p.Results.strategy
                            ...
                            case 'shift'    % shift data based on difference in overlap region
                                shift = zeros(2,size(mid1Data,2));
                                for jj = 1:size(mid1Data,2)
                                    shift(1,jj) = mean(mid1Data((mid1Data(:,jj)~=0),jj));   %average data in overlap region for each delay, 1st grating position
                                    shift(2,jj) = mean(mid2Data((mid2Data(:,jj)~=0),jj));   %average data in overlap region for each delay, 2nd grating position
                                end
                                    shift = shift - mean(shift,1);  %find signed difference between mean in overlap regions and the midpoint between them
                                    noshift = find(isnan(shift(1,:)).*isnan(shift(2,:)));   %flag any NaN values
                                    shift(noshift,:) = zeros(length(noshift),2);
                                    lowData = lowData - shift(1,:);         % do the shift operation
                                    mid1Data = mid1Data - shift(1,:);
                                    mid2Data = mid2Data - shift(2,:);
                                    highData = highData - shift(2,:);
                                    midData = [mid1Data, mid2Data];
                                    midData = reshape(midData, size(midData,1), size(midData, 2)/2, 2);
                                    midData = mean(midData, 3, 'omitnan');
                        end
...
%             end
    ...
    %% Database methods
    methods
        function stageTable = dbStageSave(obj, varargin)
% DBSTAGESAVE generates tables that will be commited to the database based
% on user input and the object array contents. This method does not 
% actually save data or commit data. Use obj.dbCommitSave(__) to save and 
% commit data.
%
% stageTable = obj.dbStageSave(connObj, dbTableName)
%   Generates a MATLAB table stageTable by using the contents of the obj 
%   array, database connection object (see db connectivity toolkit), and
%   the target database table, dbTableName. procdTable contains columns:
%   'FileName', 'FilePath','ShortName', and 'FileTypeID'. The Filename is 
%   autogenerated from the time stamp (now) and obj.shortName. The default 
%   file path is the current working directory. The file path is always 
%   returned as an absolute path with the platform specific file seperator.
%   File type is always set to 'tsObj'.
%
%   Note: this method requires that dbTableName contains the same column 
%   names as returned by procdTable.
%
% stageTable = obj.dbStageSave(__, 'name1','value1',...)
%   Same call as above but with optional name-value pairs:
%   'keepFileName': (logical) flag on whether to keep the exising file name
%       for any rows flagged for update. Any rows flagged for insert will 
%       still have file names generated by the timestamp, prefix, and 
%       shortName. The default value is true.
%   'keepFilePath': (logical) flag on whether to keep the exising file path
%       for any rows flagged for update. Any rows flagged for insert will 
%       still have file names generated by the timestamp, prefix, and 
%       shortName. The default value is ture.
%   'path': (char array) specifying the absolute data save path. Relative 
%       paths will throw an error. Default is the current working 
%       directory, pwd. 
%   'prefix': (char array) a prefix to add to shortName before generating 
%       file paths and the shortName column. Default is no prefix, ''.
%   'update': (double vector of integers) ID values that specify which rows
%       to update in the database. By default, obj.dbCommitSave(...) calls 
%       the SQL INSERT statement to add data to the database. Assigning an 
%       array to 'update' flags obj.dbCommitSave(...) to call SQL UPDATE 
%       statement as well. Use NaN as a flag for obj.dbCommitSave(...) to 
%       INSERT the specific row instead of UPDATE. Mixed numeric and NaN
%       values are allowed, which causes obj.dbCommitSave(...) to INSERT 
%       and UPDATE accordingly.
%   'colStruct': (struct) that sets values for columns in dbTableName for 
...
%       array. The structure has fields:
%           'fkCol' the name of the foreign key column in dbTableName
%           'pkTable' the name of the primary key table being reference by
%               the foreign key column
%           'pkCol' the name of the primary key column being referenced by
%               the foreign key column
%           'WHERE' the WHERE statement that filters the primary key table
%               for the correct foreign key value
%       Example: if you want to set the user to 'Ilya', but the column is a
%       foreign key called UserID pointing to the ID column in table Users,
%       you would pass as an argument:
%           genFKStruct('UserID','Users','ID','FirstName','Ilya')
%
% See Also: GENFKSTRUCT, DATABASE, DBCOMMITSAVE
            ...
            p.addParameter('keepFileName',true); %for rows flagged for update, keeps the exisiting file name
            p.addParameter('keepFilePath',true); %for rows flagged for update, keeps the exisiting file name
            p.addParameter('update',[]);    %assigns the ID column, which flags commit to update of insert in the database
            ...
            colNames = {'ID','FileName','FilePath','ShortName','Description','FileTypeID'}; %Cols that are autofilled
            ...
            % Before starting the stage, check to make sure database------
            % tables match what is being requested in the stage
            
            % Fetch table information to ensure that the table contains valid column names
            conn = p.Results.conn;
            tmpTable = conn.fetch(['SELECT TOP 1 * FROM ', p.Results.procDataTable]);
            ...
            % Generate ID column for update
            if isempty(p.Results.update)
                IDs = nan(objNumel,1);
            else
                assert(numel(p.Results.update) == objNumel, 'The size of the ''update'' array does not match the number of object elements.');
                IDs = p.Results.update(:);
            end
            ...
            if (p.Results.keepFileName || p.Results.keepFilePath) && any(~isnan(IDs))
                ...
                fileNameTable(inds,:) = fileNameTable;
                
                % Update file names
                if p.Results.keepFileName
                    fileNames(~isnan(IDs)) = fileNameTable.FileName;
                end
                
                % Update file paths
                if p.Results.keepFilePath
                    % Convert table FilePath to cell for easy processing
                    tmpFilePath = fileNameTable.FilePath;
                
                    % Convert all paths to use the platform specific file sep
                    % and prefix repoPath if the path is relative
                    isAbs = isAbsolutePath(tmpFilePath);
                    for fInd = 1:numel(tmpFilePath)
                        tmpSplitPath = splitPath(tmpFilePath{fInd});
                        if isAbs(fInd)  %absolute path, use split path
                            args = tmpSplitPath(:);
                        else %relative path, prepend repoPath
                            args = [{repoPath()}; tmpSplitPath(:)];
                        end

                        % Build full path back again using platform seperator
                        tmpFilePath{fInd} = fullfile(args{:});
                    end
                    
                    % Finally, assign updated file path
                    filePaths(~isnan(IDs)) = tmpFilePath;
                end
            end
            
            % Ensure all paths are absolute
            assert(all(isAbsolutePath(filePaths)),'Encountered a relative file save path. Absolute paths are required.');
            ...
            % Generate description cell array
            descriptions = {obj.description}';
            
            % Build table input cell array
            tableInputs = {IDs,fileNames,filePaths,shortNames,descriptions,fileTypeID};
            ...
            stageTable = table(tableConstructor{:});
            stageTable.Properties.Description = p.Results.procDataTable;
            
            % Remove ID column if set to NaN (for display purposes)
            if all(isnan(stageTable.ID))
                stageTable = removevars(stageTable,{'ID'});
            end
            
        end
        
        function [commitTable, IDStr] = dbCommitSave(obj, conn, stageTable) 
% DBCOMMITSAVE commits object data to file and the database based on the
% entries given by the output of obj.dbStageSave(...). This method first
% saves the .mat files and then commits the data to the database.
%
% [commitTable, IDStr] = obj.dbCommitSave(conn, stageTable)
%   Saves .mat files and commits their metadata in stageTable to the 
%   database given by the database connection object, conn. stageTable is a
%   table object returned by obj.dbStageSave(...). Before calling this
%   commit method, double check that stageTable has the correct entries.
%   This method also returns a commitTable, which is copy of the target
%   database table with all columns. commitTable contains updated ID and 
%   TimeStamp columns. 
%
%   In addition, this method returns IDStr, which is a char array that 
%   reconstructs the ID column in commitTable. After executing a commit, 
%   copy and paste the IDStr into the 'update' name-value pair if you want 
%   the dbStageSave method to update already commited data instead of 
%   inserting new rows. 
%
%   For example:
%       [commitTable, IDStr] = myObj.dbCommitSave(conn, stageTable); 
%   returns '123:129' as the IDStr. Copy '123:129' into the dbStageSave 
%   method call as: 
%       stageTable = obj.dbStageSave(__, 'update',123:129);
%   if you want future calls to update rows 123:129 instead of inserting 
%   additional rows.
% 
% Notes:
%
% This method individually saves elements from the object array to a .mat
% file specified by the FileName and FilePath columns of the input table.
% The object variable name is 'obj' and is a 1x1 object of the same type as
% the input object array.
%
% This method commits data to the database by executing a parameterized SQL
% INSERT and/or UPDATE statement, depending on whether a row has an ID
% assigned to it or not by obj.dbStageSave(...). For rows that do not have
% an assigned ID (missing ID column or set to NaN), the INSERT statement is
% called. For rows that do have an assigned ID, the UPDATE statement is
% called on the primary key ID column.
%
% This method also modifies the path entires commited to the database. All
% path file seperators (except for root on MacOS or Linux, '/') are
% converted to the Windows seperator, '\'. If the file path is common to
% the repository path returned by repoPath(), the file path is truncated to
% only contain the relative path from the repository path. Otherwise, the
% path is kept as an absolute path.
%
% In addition, the database commit updates the table/row TimeStamp to the 
% timestamp of the commit.
%
% Finally, this method will rollback any changes made to the database if an
% error is thrown before committing all rows. Any files saved in the
% process will need to be manually deleted. Also, the rollback typically 
% does not undo any increments to autonumber or autoindex fields.
%
% This method has only been tested with MS Access ODBC providers on Windows
% platforms.
%
% Known Issues: 
%   -MS Access does not support rollback.
%   -MS Access incorrectly returns committed data when first committing to
%   an empty table
%
% See Also: database, dbStageSave

            % Format object array dims into a column for easy looping
            objNumel = numel(obj);
            obj = obj(:);
            
            % Loop over object data, modify table paths, and save data to file
            for objInd = 1:objNumel
                % Save object data to file
                savePath = fullfile(stageTable.FilePath{objInd},stageTable.FileName{objInd});
                saveData = struct('obj',obj(objInd));
                save(savePath,'-struct','saveData');
                
                % When paths are part of the repo, they need to be relative to the repo path
                tmpPath = splitPath(stageTable.FilePath{objInd});    %remove any paltform-specific seperators
                [~,stageTable.FilePath{objInd}] = comparePaths(fullfile(tmpPath{:}), repoPath,'\'); %construct the relative path using windows seperator
            end
            
            % Split the table into UPDATE and INSERT rows based on ID column
            if ~ismember('ID',stageTable.Properties.VariableNames) % there is no ID column, everything is an INSERT statement
                updateTable = table();  %table for UPDATE statement 
                insertTable = stageTable; %table for INSERT statement
                insertInds = true(objNumel,1); %elements of stageTable that call INSERT
            else
                insertInds = isnan(stageTable.ID); %elements of stageTable that call INSERT
                updateTable = stageTable(~insertInds,:); %table for UPDATE statement
                insertTable = stageTable(insertInds,:); %table for INSERT statement
                insertTable = removevars(insertTable,{'ID'});   % remove ID column for the INSERT table to auto-assign ID
            end
            
            % Use a try catch statement to only commit db changes if this
            % method finishes without error. Any error rolls back changes.
            try
                % Set autocommit to off and manually commit changes at the
                % end of the try clause
                commitStatus = conn.AutoCommit;
                conn.AutoCommit = 'off';
                
                % Start the commit table, which is a fetched copy of the table
                % committed to the database
                commitTable = conn.fetch(['SELECT TOP 1 * FROM ' stageTable.Properties.Description ';']);
                commitTable.Properties.Description = stageTable.Properties.Description;
                
                % UPDATE statement
                if any(~insertInds)
                    % fetch the ID values and remove from updateTable
                    IDs = updateTable.ID;
                    updateTable = removevars(updateTable,{'ID'});
...
%                     end
                    ...
                    % to work with MS Access and has only been tested with
                    % MS Access/ODBC drivers
                    varNames = escapeReservedWords(varNames,conn.DatabaseProductName);
                                        
                    % Loop through the update rows and update one at a time
                    for rowInd = 1:size(updateTable,1)
                        % Values as a cell--required to escape reserved words
...
%                         end 
                        ...
                        conn.update(updateTable.Properties.Description,...
                                    varNames, varValues,...
                                    ['WHERE ID = ' num2str(IDs(rowInd))]);
                        
                        % Get column information from most recently updated record
                        tmp = conn.fetch(['SELECT * FROM ' updateTable.Properties.Description ' WHERE ID = ' num2str(IDs(rowInd)) ';']);
                        commitTable = [commitTable; tmp];
                    end
                end
                ...
                if any(insertInds)
                    ...
                    conn.sqlwrite(insertTable.Properties.Description,insertTable);

                    % Query the database for the commited table for return argument
                    tmp = conn.fetch(['SELECT TOP ', num2str(size(insertTable,1)) ' * FROM ',  stageTable.Properties.Description, ' ORDER BY ID DESC;']);
                    tmp = sortrows(tmp,'ID');
                    commitTable = [commitTable; tmp];
                end

%                 rollback(conn);
%                 conn.AutoCommit = commitStatus;
                
                % Rearrange commit table to match the stageTable order
                commitTable = commitTable(2:end,:); % drop 1st entry--determines datatypes
                tmp = commitTable;  %use a copy to rearrange indicies
                
                % rows modified by UPDATE
                if any(~insertInds)
                    commitTable(~insertInds,:) = tmp(1:sum(~insertInds),:);
                end
                
                % rows generated by INSERT
                if any(insertInds)
                    commitTable(insertInds,:) = tmp((sum(~insertInds)+1):end,:);
                end
                
                % Assume ID is numeric and generate an array to return to the
                % user so they can use the 'update', inds, name-value pair when
                % staging the data
                IDStr = vect2colon(commitTable.ID');
                
            catch ME
                % An error occured above. Before throwing the exception, 
                % rollback changes made to the DB and reset autocommit state
                rollback(conn);
                conn.AutoCommit = commitStatus;
                throwAsCaller(ME);
            end
            
            % Commit changes to the database once all code executed without error
            commit(conn);
            conn.AutoCommit = commitStatus;
            
        end
        
        function [stageTable, stageView] = dbStageAssoc(obj, varargin)
% DBSTAGEASSOC generates associative tables that will be commited to the 
% database based on user input and the object array contents. The table
% generated by this method links the processed data saved by
% obj.dbCommitSave(__) to its corresponding raw data. This method does not 
% commit data to the database. Use obj.dbCommitAssoc(__) to commit data to 
% the database.
%
% stageTable = obj.dbStageAssoc(connObj, assocTable, rawTable,...
%                               procdTable, commitTable);
%   Generates a MATLAB table stageTable that can be committed to assocTable
%   by using inputs: 
%       1. obj array, (transientspectra obj) This method uses the name 
%          field of obj, which contains the raw files used to initially 
%          load the object data
%       2. connObj, (database connection object) that connects to the
%          database. See database for more detail.
%       3. assocTable, (char) the name of the associative table, typically
%          assoc[dataLogPrefix][rawSuffix][procdSuffix]. For example,
%          assocTRAcquisitionProcd or assocTRAcquisitionPhononRemoved
%       4. rawTable, (char) the name of the raw data table used to initially load 
%          object data. For example, TRAcquisition
%       5. procdTable, (char) the name of the processed data table used to commit
%          processed data. For example, TRProcd or TRPhononRemoved
%       6. commitTable, (MATLAB table) a commit or stage table that
%          contains a fully populated ID column (no NaN values). Typically,
%          this would be the commitTable output of dbCommitSave(...), 
%          although the stageTable output of dbStageSave(...) also works if
%          the 'update' name-value pair is set.
%   By default, this method call attempts to assign rawTable and procdTable
%   to their FK columns in assocTable by using the naming convention:
%       raw FK column -> [singularRawTable]ID
%       procd FK column -> [singularProcdTable]ID
%   If the column names in assocTable do not follow this naming convention
%   or if the rawTable or procdTable have complex plural forms (e.g. is not
%   names->name or tables->table), this method will return a warning and
%   assume that the FK column order in assocTable is rawTableID followed by
%   procdTableID. This assumption can be overriden with the 'colOrder'
%   name-value pair.
%
% stageTable = obj.dbStageAssoc(__, 'name1','value1',...)
%   Same call as above but with optional name-value pairs:
%   'colOrder': (double vector of integers) Overrides the default assoc 
%       table column order of rawTableID, procdTableID when singular table
%       names cannot be geneerated or when the column name does not follow
%       the encoded naming convention. For example, if your column names 
%       are ID, tableAsID, and tableBsID, where tableAs contains procd data
%       and tableBs contains raw data, specify 'colOrder',[1,3,2] to 
%       reverse the raw and procd table orders.
%   'update': (double vector of integers) ID values that specify which rows
%       to update in the database. By default, obj.dbCommitAssoc(...) calls 
%       the SQL INSERT statement to add data to the database. Assigning an 
%       array to 'update' flags obj.dbCommitAssoc(...) to call SQL UPDATE 
%       statement as well. Use NaN as a flag for obj.dbCommitAssoc(...) to 
%       INSERT the specific row instead of UPDATE. Mixed numeric and NaN
%       values are allowed, which causes obj.dbCommitAssoc(...) to INSERT 
%       and UPDATE accordingly.
%
% [stageTable, stageView] = obj.dbStageAssc(__)
%   Additionally returns a stageView, which is a MATLAB table that contains
%   and ID column, raw and procd data FK columns, and the FileName column 
%   values in the corresponding PK tables. The FK column names are the
%   column names in assocTable, while the PK FileName columns are the PK
%   table names suffixed with FileName. Use this table to verify that the
%   correct file names are being associated.
%
% See Also: database, dbCommitAssoc, dbCommitSave, dbStageSave          
            
            % parse required and optional user inputs
            p = inputParser();
            
            p.addRequired('conn');          %The database connection object
            p.addRequired('assocTable');    %the name of the table that logs processed data
            p.addRequired('rawTable'); %the name of the table that logs processed data
            p.addRequired('procdTable');  %the name of the table that logs processed data
            p.addRequired('commitTable');   %the commit table generated by dbCommitSave
            
            p.addParameter('update',[]);    %assigns the ID column, which flags commit to update of insert in the database
            p.addParameter('colOrder',[]);   %flips column order if assoc col names does not follow naming scheme
            ...
            % Generate a list of columns to include into the commit--------
            inputTableNames = {p.Results.rawTable ,p.Results.procdTable};
            colNames = [{'ID'}, inputTableNames];
            
            % Before starting the stage, check to make sure database------
            % tables match what is being requested in the stage. If does
            % not match, warn user.
            
            % Fetch table information to ensure that the table contains valid column names
            conn = p.Results.conn;
            tmpTable = conn.fetch(['SELECT TOP 1 * FROM ', p.Results.assocTable]);
            
            % Generate raw and procd table names by removing plurals and suffixing ID
            % e.g. 'TRProcd' - > 'TRProcdID'
            % e.g. 'BeamSizes' - > 'BeamSizeID'
            colNames(2:end) = strcat(cellfun(@(c) c{1}, regexp(colNames(2:end),'\w+(?=s$)|\w+','match'), 'UniformOutput', false),'ID');
            ...
            for ii = 1:numel(colNames)
                ...
                if ~any(inds)
                    if ii==1
                        error('ID Column required in table %s.', p.Results.assocTable);
                    else
                        warning('Col %s was not found in table %s.',colNames{ii}, p.Results.assocTable); %assert an index exists
                    end
                else
                    colOrder(ii) = find(inds); %assigns index position to colOrder
                end
            ...
            % If any columns were missing, use database order and assume
            % raw is 1st and procd is 2nd
            if any(colOrder==0)
                if isempty(p.Results.colOrder)
                    colNames(2:end) = pTCols(2:numel(colOrder));
                    colOrder(2:end) = 2:numel(colOrder);
                else
                    assert(numel(colNames)==numel(p.Results.colOrder) && max(p.Results.colOrder)<=numel(pTCols),...
                        '''colOrder'' must have the same number of indicies as commitNames, %d indicies', numel(colNames));
                    colNames = pTCols(p.Results.colOrder);
                    colOrder = p.Results.colOrder;
                end
            end
            
            % Generate a list of raw file names paired --------------------
            % with the procd commit ID
            rawFileNames = cell(objNumel,1);
            procdFileIDs = cell(objNumel,1);
            
            % Loop over object and construct nested cell of raw file names
            for objInd = 1:objNumel
                % retrieve raw file name as char or cell array
               rawFileNames{objInd} = obj(objInd).name;
               
               % convert char to cell array
               if ischar(rawFileNames{objInd})
                   rawFileNames{objInd} = rawFileNames(objInd);
               end
               
               % remove any duplicate entries
               rawFileNames{objInd} = unique(rawFileNames{objInd});
               
               % ensure procd commit ID cell nest also matches size of rawFileName cell 
               % so there is always a 1 to 1 correspondence
               procdFileIDs{objInd} = repmat({p.Results.commitTable.ID(objInd)},numel(rawFileNames{objInd}),1);
            end
            
            % Convert nested cells into cell vector and numeric vector
            rawFileNames = vertcat(rawFileNames{:});    % cell vector
            procdFileIDs = cell2mat(vertcat(procdFileIDs{:}));    %numeric vector
            nFileNames = numel(rawFileNames);
            
            % Remove any absolute path names from rawFileNames
            for fInd = 1:nFileNames
                 [~,fName,fExt] = fileparts(rawFileNames{fInd});
                 rawFileNames{fInd} = [fName, fExt];
            end
            
            % Generate ID column for update -------------------------------
            if isempty(p.Results.update)
                IDs = nan(nFileNames,1);
            else
                assert(numel(p.Results.update) == nFileNames, 'The size of the ''update'' array does not match the number of raw file names in obj.name, %d.', nFileNames);
                IDs = p.Results.update(:);
            end
            
            % Query for raw file ID names ---------------------------------
            rawFileIDs = zeros(nFileNames,1);
            for rawInd = 1:nFileNames
                % Query for FileName from rawTable
                tmpTable = conn.fetch(sprintf('SELECT ID, FileName FROM %s WHERE FileName = ''%s''', p.Results.rawTable, rawFileNames{rawInd}));
                
                % Make sure there is only one result, and return an error if less or warning if more
                assert(~isempty(tmpTable.ID),'Could not find FileName %s in table %s.',rawFileNames{rawInd},p.Results.rawTable);
                if numel(tmpTable.ID)>1
                    warning('Multiple FileNames were found for %s in table %s. Using the 1st match for the assocTable.', rawFileNames{rawInd}, p.Results.rawTable);
                end
                
                % Assign rawFileID
                rawFileIDs(rawInd) = tmpTable.ID(1);
            end
            
            % Build table input cell array
            tableInputs = {IDs(:),rawFileIDs(:),procdFileIDs(:)};
            ...
            % Build stage table--table throws error if there are duplicate column/variable names
            tableConstructor = [tableInputs(:); {'VariableNames'}; {colNames}];
            stageTable = table(tableConstructor{:});
            stageTable.Properties.Description = p.Results.assocTable;
            
            % Construct a verification table that returns both IDs and FileNames
            rawQueryNames = cell(size(stageTable,1),1);
            procdQueryNames = cell(size(stageTable,1),1);
            
            % Loop over stageTable rows and query the database for filename
            % This is a bit redundant, but forces an independent verification of the code and user entries
            for rowInd = 1:size(stageTable,1)
                % Query for FileName from rawTable
                tmpTable = conn.fetch(sprintf('SELECT ID, FileName FROM %s WHERE ID = %d', inputTableNames{1}, stageTable{rowInd,colOrder(2)}));
                rawQueryNames{rowInd} = tmpTable.FileName(1);
                
                % Query for FileName from procdable
                tmpTable = conn.fetch(sprintf('SELECT ID, FileName FROM %s WHERE ID = %d', inputTableNames{2}, stageTable{rowInd,colOrder(3)}));
                procdQueryNames{rowInd} = tmpTable.FileName(1);
            end
            
            % Build verification table (stageView)--table throws error if there are duplicate column/variable names
            tableInputs = {stageTable{:,1},stageTable{:,colOrder(2)},rawQueryNames(:),stageTable{:,colOrder(3)},procdQueryNames(:)};
            viewColNames = {colNames{1},colNames{colOrder(2)},[inputTableNames{1} 'FileName'],colNames{colOrder(3)},[inputTableNames{2} 'FileName']};
            tableConstructor = [tableInputs(:); {'VariableNames'}; {viewColNames}];
            stageView = table(tableConstructor{:});
            stageView.Properties.Description = p.Results.assocTable;
        ...
        function [commitTable, IDStr] = dbCommitAssoc(obj, conn, stageTable)
% DBCOMMITASSOC commits object associations (procd file to raw files) to 
% the database based on the entries given by the output of 
% obj.dbStageAssoc(...). 
%
% [commitTable, IDStr] = obj.dbCommitAssoc(conn, stageTable)
%   Commits object associations (procd file to raw files) in stageTable to 
%   the database given by the database connection object, conn. stageTable 
%   is a table object returned by obj.dbStageAssoc(...). Before calling 
%   this commit method, double check that stageTable and has the correct 
%   entries. This method also returns a commitTable, which are copies of 
%   the target database table with all columns, with their FK values 
%   commitTable contains the committed ID columns. 
%
%   In addition, this method returns IDStr, which is a char array that 
%   reconstructs the ID column in commitTable. After executing a commit, 
%   copy and paste the IDStr into the 'update' name-value pair if you want 
%   the dbStageAssoc method to update already commited data instead of 
%   inserting new rows. 
%
%   For example:
%   commitTable, commitView, IDStr] = myObj.dbCommitAssoc(conn,stageTable); 
%   returns '1:4' as the IDStr. Copy '1:4' into the dbStageAssoc 
%   method call as: 
%       stageTable = obj.dbStageAssoc(__, 'update',1:4);
%   if you want future calls to update rows 1:4 instead of inserting 
%   additional rows.
% 
% Notes:
%
% This method commits data to the database by executing a parameterized SQL
% INSERT and/or UPDATE statement, depending on whether a row has an ID
% assigned to it or not by obj.dbStageAssoc(...). For rows that do not have
% an assigned ID (missing ID column or set to NaN), the INSERT statement is
% called. For rows that do have an assigned ID, the UPDATE statement is
% called on the primary key ID column.
%
...
%   an empty table
%
% See Also: dbStageAssoc

            % Number of stage entries
            stageNumel = size(stageTable,1);
            
            % Split the table into UPDATE and INSERT rows based on ID column
            if ~ismember('ID',stageTable.Properties.VariableNames) % there is no ID column, everything is an INSERT statement
                updateTable = table();  %table for UPDATE statement 
                insertTable = stageTable; %table for INSERT statement
                insertInds = true(stageNumel,1); %elements of stageTable that call INSERT
            else
                insertInds = isnan(stageTable.ID); %elements of stageTable that call INSERT
                updateTable = stageTable(~insertInds,:); %table for UPDATE statement
                insertTable = stageTable(insertInds,:); %table for INSERT statement
