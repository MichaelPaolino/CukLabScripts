...
%   'keepName': (logical) flag on whether to keep the exising file name and
%       file path for any rows flagged for update. Any rows flagged for
%       insert will still have file names generated by the timestamp,
%       prefix, and shortName. The default value is ture.
...
% See Also: GENFKSTRUCT, DATABASE, DBCOMMITSAVE
            ...
            p.addParameter('keepName',true); %for rows flagged for update, keeps the exisiting file name
            ...
            if p.Results.keepName && any(~isnan(IDs))
                ...
                filePaths(~isnan(IDs)) = fileNameTable.FilePath;
                
                % Convert all paths to use the platform specific file sep
                % and prefix repoPath if the path is relative
                isAbs = isAbsolutePath(fileNames);
                for fInd = 1:numel(fileNames)
                    tmpSplitPath = splitPath(filePaths{fInd});
